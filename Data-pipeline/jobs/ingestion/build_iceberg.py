...
parser.add_argument("--table", required=True, help="token_transfers | tokens | transactions | blocks")
...
bronze_base = f"gs://{bucket}/bronze/{table}"
...
try:
    ...
    row_count = src.count()
    print(f"Bronze rows to merge: {row_count}")
    if row_count == 0:
    ...
    if is_snapshot:
        # Unpartitioned dimension, full snapshot replace. No extensions needed.
        ...
        # Idempotent per-day reload: overwrite exactly the day partitions
        # present in src. No SQL MERGE, no session extensions needed.
        src.writeTo(target).overwritePartitions()
        print(f"Overwrite partitions into {target} complete")
